...
basic_integration_tests = [
    ...
    {"name": "folder.get_datamanagers",
     ...
     "check": lambda x: isinstance(x, list) and sorted(x) == [(username, "tempZone") for username in ["bob@yoda.dev", "datamanager", "datamanager@yoda.test", "functionaladminpriv", "functionaladminpriv@yoda.test"]]},
    ...
    {"name":  "folder.determine_new_vault_target.invalid",
     ...
     "check": lambda x: x == ""},
    {"name":  "groups.get_categories",
     "test": lambda ctx: groups.get_categories(ctx),
     "check": lambda x: sorted(x) == sorted(['test-automation', 'System', 'intake-intake2', 'intake-intake', 'datarequests'])},
    {"name":  "groups.get_datamanager_categories.datamanager",
     "test": lambda ctx: groups.get_datamanager_categories(ctx, "datamanager"),
     "check": lambda x: sorted(x) == sorted(['test-automation'])},
    {"name":  "groups.get_datamanager_categories.researcher",
     "test": lambda ctx: groups.get_datamanager_categories(ctx),
     "check": lambda x: sorted(x) == sorted([])},
    {"name":  "groups.get_datamanager_categories.rods",
     "test": lambda ctx: groups.get_datamanager_categories(ctx),
     "check": lambda x: sorted(x) == sorted([])},
    {"name":  "groups.get_subcategories.test_automation",
     "test": lambda ctx: groups.get_subcategories(ctx, "test-automation"),
     "check": lambda x: sorted(x) == sorted(['data-archive-geo', 'initial', 'metadata-schemas', 'smoke-test', 'test-automation'])},
    {"name":  "groups.get_subcategories.doesnotexist",
     "test": lambda ctx: groups.get_subcategories(ctx, "doesnotexist"),
     "check": lambda x: x == []},
    ...
    {"name": "statistics.get_user_groups_for_stats.researcher",
     ...
     "check": lambda x: sorted(x) == sorted(['deposit-pilot', 'grp-intake-initial', 'grp-intake-test', 'intake-test2', 'research-core-0', 'research-core-1', 'research-core-2', 'research-dag-0', 'research-default-0', 'research-default-1', 'research-default-2', 'research-default-3', 'research-epos-msl-0', 'research-epos-msl-1', 'research-hptlab-0', 'research-hptlab-1', 'research-initial', 'research-initial1', 'research-revisions', 'research-smoke-test', 'research-teclab-0', 'research-teclab-1', 'research-vollmer-0'])},
    {"name": "statistics.get_user_groups_for_stats.datamanager",
     "test": lambda ctx: get_user_groups_for_stats(ctx, user_name="datamanager"),
     "check": lambda x: sorted(x) == sorted(['grp-datamanager-initial', 'grp-datamanager-test', 'grp-datamanager-test2', 'grp-intake-initial', 'grp-intake-test', 'intake-test2', 'research-smoke-test', 'deposit-pilot', 'deposit-pilot1', 'research-core-0', 'research-core-1', 'research-core-2', 'research-dag-0', 'research-default-0', 'research-default-1', 'research-default-2', 'research-default-3', 'research-epos-msl-0', 'research-epos-msl-1', 'research-hptlab-0', 'research-hptlab-1', 'research-initial', 'research-initial1', 'research-revisions', 'research-teclab-0', 'research-teclab-1', 'research-vollmer-0'])},
...
def _test_collection_subcollections(ctx: rule.Context) -> List[str]:
    ...
    if result_nosub_nonr != []:
        result.append("test_collection_subcollections fail on empty/NR: " + str(result_nosub_nonr))

    result_nosub_rec = list(collection.subcollections(ctx, os.path.join(basepath, "a1/a2"), True))
    if result_nosub_rec != []:
        result.append("test_collection_subcollections fail on empty/R: " + str(result_nosub_rec))

    result_l1_nonr = list(collection.subcollections(ctx, os.path.join(basepath, "a1")))
    if result_l1_nonr != [os.path.join(basepath, "a1/a2")]:
        result.append("test_collection_subcollections fail on L1/NR: " + str(result_l1_nonr))
    ...
    if sorted(result_l2_rec) != sorted([os.path.join(basepath, dir) for dir in testdirs]):
        result.append("test_collection_subcollections fail on L2/R: " + str(result_l2_rec))
